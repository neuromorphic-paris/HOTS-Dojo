...
#
# tsurface : matrix returned with dimx, dimy as dimensions
# =============================================================================
def Time_Surface_all(xdim, ydim, timestamp, timecoeff, dataset, minv=0.1, verbose=False):
    tmpdata = [dataset[0].copy(), dataset[1].copy()]
    #taking only the timestamps after the reference 
    ind_subset = tmpdata[0]>=timestamp
    tmpdata = [tmpdata[0][ind_subset], tmpdata[1][ind_subset]]    
    ...
    for i in range(len(tsurface_array)):
        if tsurface[tmpdata[1][i][1],tmpdata[1][i][0]]==0:
            tsurface[tmpdata[1][i][1],tmpdata[1][i][0]]=tsurface_array[i]  
...
# =============================================================================
def Time_Surface_event(ldim, event, timecoeff, dataset, minv=0.1, verbose=False):
    tmpdata = [dataset[0].copy(), dataset[1].copy()]
    ...
    #Note that exp is monotone and the timestamps are ordered, thus the last 
    #values of the dataset will be the lowest too
    tsurface = np.zeros([ldim,ldim])
    offs = np.int(np.floor(ldim/2))
    for i in range(len(tsurface_array)):
        if tsurface[tmpdata[1][i][1]+offs,tmpdata[1][i][0]+offs]==0:
            tsurface[tmpdata[1][i][1]+offs,tmpdata[1][i][0]+offs]=tsurface_array[i]  
    #plot graphs if verbose is set "True"
    if (verbose==True):
        plt.figure()
        sns.heatmap(tsurface)
